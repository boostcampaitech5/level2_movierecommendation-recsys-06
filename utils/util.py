...
def get_raw_data(settings) -> pd.DataFrame:
    ...
    raw_input_df: pd.DataFrame = pd.read_pickle(os.path.join(data_path, df_path[0]))
    ...
    return raw_input_df, raw_user_df, raw_item_df
...
def create_data(settings):
    ...
    input_df.rename(columns={'user': 'user_id', 'item': 'item_id', 'time': 'unix_time'}, inplace=True)
    input_df['date_actual'] = input_df['unix_time'].apply(lambda x: datetime.utcfromtimestamp(x).strftime('%Y%m%d%H%M%S'))
    ...
    user_df['item_watched_num'] = user_df['user_id'].map(input_df.groupby('user_id')['item_id'].count())
    ...
    item_df['user_watched_num'] = item_df['item_id'].map(input_df.groupby('item_id')['user_id'].count())
    ...
    for tsv in tsv_list:
        ...
        if tsv not in not_idx_list:
            indexing_dict[tsv] = {v: i for i, v in enumerate(sorted(tsv_file[tsv[:-1]].unique()))}
            tsv_file[tsv[:-1]] = tsv_file[tsv[:-1]].map(indexing_dict[tsv])

        grouped_file = tsv_file.groupby("item").apply(lambda x: x[tsv[:-1]].values.squeeze().tolist())
        if tsv not in not_idx_list:
    ...
    total_df = input_df.merge(item_df, on='item_id')
    total_df = total_df.merge(user_df, on='user_id')

    total_df.to_pickle(os.path.join(settings['path']['data'], "total_data.pkl"))

    return user_df, item_df, total_df
